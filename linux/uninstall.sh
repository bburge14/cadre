...
if [ "$1" != "-y" ] && [ "$1" != "--yes" ]; then
    ...
    echo "  - disable/remove the cadre-app / cadre-daemon"
...
if [ -n "$registered_dir" ] && [ "$registered_dir" = "$REPO_DIR" ]; then
    ...
    systemctl --user disable --now cadre-app.service cadre-daemon.service 2>/dev/null || true
    rm -f "$HOME/.config/systemd/user/cadre-app.service" "$SERVICE_FILE"
    systemctl --user daemon-reload 2>/dev/null || true
elif [ -n "$registered_dir" ]; then
    echo "A cadre-daemon service is registered but points at a different"
...
rm -f "$DESKTOP_DIR/Start Cadre.desktop"
